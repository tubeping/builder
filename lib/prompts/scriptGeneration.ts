...
## 🔢 수치 가드 — 범위는 좁게, 살갗은 자유롭게

### 자리표시 [___] 강제 (반드시 입력에 있어야 함)
다음 4종만 입력에 명시 없으면 **반드시 [___]**:
- **가격** (정가 / 공구가 / 할인율)
- **공구 기간** (시작일 / 마감일 / 시각)
- **재고 수량** (한정 N개)
- **할인 폭** (N% 할인 / N원 절감)

→ 이 4종은 사용자가 직접 채워야 하는 사실 정보. 모델이 추측 금지.

### 자유롭게 풍부하게 채워도 OK (사실 단정만 피할 것)
- 감각 묘사 (식감·향·맛·촉감·소리·시각적 인상)
- 사용 상황 묘사 ("출근길에", "지하철에서", "엄마랑 같이")
- 카테고리 일반 셀링 포인트 (망고면 "달콤한 과즙", 영양제면 "한 알의 편의")
- 시청자 감정 어휘 ("기다리신 분들", "고민하셨던 분")
- 후기 톤 ("저도 먹어봤는데...")

### 추상 형용사 — 가능하면 구체로 바꾸되, 한두 개는 OK
- "정말 / 너무 / 굉장히 / 꿀처럼" 등은 **3섹션 통틀어 1~2번만 허용**
- 더 좋은 표현: 구체 비유 ("입에 넣으면 과즙이 터지듯", "한 입에 사라지는")

### 핵심 원칙
**키워드만 받았어도 본문은 풍부하게 채우세요.**
입력이 "태국 망고"뿐이어도 → 감각 묘사·일반 셀링 포인트로 자연스럽게 살을 붙이세요.
빈약한 본문보다 풍부한 본문이 낫고, 거기에 가격·기간 자리표시 [___] 만 깔끔히 박혀 있으면 됩니다.
...
2. core — 10~40초, 제품 핵심 장점 + 공구 조건 + **신뢰 신호 반복** (앵커링 포함)
3. cta — 2~7초, **시한성 필수** (구체 시간·날짜 — "오늘 자정", "5/24 23시 마감")

### 분량 가이드 (1분 영상 기준 — 충분히 풍부하게)
- hook: 15자 이내 (말로 1~3초)
- core: **150~350자** (말로 15~40초) — 짧으면 어색
- cta: 30~60자 (말로 3~7초)
- 전체 합계: **200~430자**

### CTA 모호 표현 금지 ⚠️
다음 표현 절대 사용 금지:
- "곧 오픈" "조만간" "곧 시작" "이른 시일 내" "다음 주쯤" "조금만 기다려"

대신 반드시 구체 시간:
- 시간 정보가 입력에 있으면 → 그대로 사용 ("5월 24일 오전 11시 오픈")
- 시간 정보 없으면 → 자리표시 **[___월 ___일 ___시]** 로 둘 것

### 규칙
- **전체가 훅이다** — 그라데이션 없음, 즉시 본론
- 장황한 인사/소개 금지
- "안녕하세요 여러분" 같은 오프닝 절대 금지
- 숫자·구체 수치 필수 (할인율·가격·기간)
- hook의 핵심 메시지를 core·cta에서도 **다른 표현으로 반복** (메시지 1개 관통)
...
1. **한 줄 메시지 결정** — 이 대본을 관통할 메시지 한 문장을 마음속으로 먼저 정하세요
2. **hook 작성** — 그 메시지의 가장 강한 변주
3. **core 작성** — 그 메시지의 증거·디테일 + **감각·상황·셀링 포인트로 살을 충분히 붙임**
4. **cta 작성** — 그 메시지의 결론 + 구체 시한 (없으면 [___])
5. **검증** — hook·core·cta 세 섹션이 같은 한 줄 메시지의 변주인지 확인

## 🍯 입력이 빈약해도 본문은 풍부하게

위 "공구 내용"이 짧거나 키워드 수준이라도 **본문은 절대 빈약하게 만들지 마세요**.
다음 방식으로 자연스럽게 살을 붙이세요:

- 카테고리 일반 셀링 포인트 — 예: 망고 → "달콤한 과즙·식감", 영양제 → "한 알의 편의"
- 감각 묘사 — 맛·향·식감·시각·촉감
- 사용 상황 — "아침에 한 알", "출근길에", "퇴근하고"
- 시청자 호명 — "기다리셨던 분들", "고민이셨던 분들"
- 후기 톤 — "저도 먹어봤는데", "써본 후기로는"

🚨 **단, 다음 4종은 입력에 없으면 반드시 [___] 자리표시**:
- 가격(정가/공구가/할인율) · 공구 기간 · 재고 수량 · 할인 폭
이건 사용자가 직접 채워야 하는 정확한 사실 정보. 추측 금지.

✅ 좋은 예 (키워드 "태국 망고"만 입력 받았을 때):
HOOK: "**한 입에 과즙 터지는 태국 망고**, 곧 만나요"
CORE: "여름 한정 태국 직수입 망고가 드디어 도착했어요. 진한 노란빛만 봐도 달콤함이 그려지고, 한 입 베어 물면 과즙이 입안 가득 퍼지더라고요. 한 박스에 가족이랑 같이 드시기 딱 좋은 양이에요. 정가 [___]원 → 공구가 [___]원."
CTA: "[___월 ___일 ___시] 오픈, 프로필 링크에서 알림 신청 먼저!"

✗ 나쁜 예 (지나치게 빈약):
CORE: "태국 망고예요. 맛있어요. 공구합니다."

## 마무리 체크리스트
- [ ] 세 섹션을 관통하는 **한 줄 메시지**가 명확한가?
- [ ] hook에 **신뢰 신호 2개**가 구체 명사로 박혀 있는가? (입력에 정보가 적으면 카테고리/원산지/일반적 셀링 포인트 2개라도)
- [ ] core가 **150~350자**로 충분히 풍부한가? (빈약하면 감각·상황·셀링 포인트로 살 추가)
- [ ] 가격·기간·재고만 [___] 자리표시 사용 (다른 모든 표현은 살 붙여도 OK)
- [ ] CTA에 "곧·조만간" 모호 표현 없는가? (구체 시간 또는 [___])
- [ ] categories.main + categories.tags 채웠는가?
`;
}
