...
1. **한 줄 메시지 결정** — 이 대본을 관통할 메시지 한 문장을 마음속으로 먼저 정하세요
   예: "백화점 절반 가격에 더 큰 망고" / "오메가3+커큐민 한 알 끝" / "비린내 없는 한의사 영양제"
2. **hook 작성** — 그 메시지의 가장 강한 변주
3. **core 작성** — 그 메시지의 증거·디테일 확장
4. **cta 작성** — 그 메시지의 결론 + 구체 시한
5. **검증** — hook·core·cta 세 섹션이 모두 같은 한 줄 메시지의 변주인지 확인. 아니면 다시.

## 마무리 체크리스트
- [ ] 세 섹션을 관통하는 **한 줄 메시지**가 명확한가? (가장 중요)
- [ ] hook에 위 상품명 또는 체험 포인트의 핵심이 들어갔는가?
- [ ] hook에 **신뢰 신호 2개 이상**이 구체 명사로 박혀 있는가? (브랜드·국가·인증·수치·실적 중)
- [ ] 본문에 위 체험·공구 조건이 그대로 반영됐는가? (AI가 지어낸 경험담 금지)
- [ ] 시스템 예시의 카테고리(다이어트·뷰티 등)로 빠지지 않았는가?
- [ ] 가격·기간 숫자는 입력한 그대로인가? (없으면 [___] 자리표시)
- [ ] CTA에 "곧·조만간·이른 시일" 같은 모호 표현 없는가? (구체 시간 또는 [___] 자리표시)
- [ ] 추상 형용사("정말·너무·꿀처럼·미친") 안 썼는가?
